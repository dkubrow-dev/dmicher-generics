...
export function appearanceHelpContent() {
  ...
  const pages = [
    { id: "welcome", title: welcomeText().name, html: p(ru
      ...
      + p(ru ? "Чтобы изменить имя отправителя, откройте «Актёры → dmicher modules → generic» и переименуйте актёра Информатора. Имя сохранится после повторного входа и появится в новых сообщениях. После удаления актёр создаётся с именем «Информатор». Суффикс в имени технического пользователя не переносится в имя актёра."
        : "To change the sender name, open Actors → dmicher modules → generic and rename the Informer actor. The name survives rejoining and appears in new messages. A deleted actor is recreated as Informer. A suffix in the technical user name is not copied to the actor name.") },
