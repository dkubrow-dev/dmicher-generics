...
Скачивание использует `application/json` и имя с обязательным расширением `.json`. Blob — внутренний транспорт браузера, а не формат результата. Собственная ссылка останавливает всплытие клика: иначе обработчик гиперссылок Foundry открывает `blob:` в новой вкладке и теряет имя файла. Стандартное скачивание браузера при этом сохраняется.
