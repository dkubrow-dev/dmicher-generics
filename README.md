...
Общий бесплатный Foundry-модуль для семейства dmicher. Он объединяет оформление и поведение окон, интерфейс справки, экранирование HTML, последовательную очередь задач, технические личности и повторяемое поведение сообщений чата. Небольшой версионируемый реестр API позволяет модулям находить друг друга; необязательный мост Premium подключает реализации объявленных методов. Контент справки, игровые правила, лицензирование и автоматизация эпизодов остаются в соответствующих модулях.
...
Устанавливаемое содержимое находится в `dmicher-generics/`; в Foundry оно помещается в `Data/modules/dmicher-generics/`. Модуль нужно включить вместе с потребителем. Он не требует Premium, системы или серверного компонента. Импорт API не создаёт документы. При загрузке мира Generics поддерживает общего Информатора и отправляет личное приветствие со списком активных модулей. Бесплатная мировая настройка приветствия не отключает сообщения потребителей. Actor Информатора хранится в `dmicher modules/generic`, старые личности других модулей не мигрируются. Телеметрии в Generics нет. Подробности: [приветствие](docs/welcome.md), [общий чат](docs/chat-api.md).
...
// На init:
windowTheme.install();
// На ready, если потребителю требуется повторное применение:
windowTheme.apply();
// При демонтаже интеграции, если он нужен:
windowTheme.dispose();
```

Темы `dark` и `light` используют CSS-переменные `--dmicher-*`. Общая клиентская настройка Generics задаёт одну тему всем окнам dmicher в этом браузере. Окно «Оформление и окна» открывается из настроек Foundry → dmicher 🧰 Generics или через `generics.appearance.openSettings()`. Разметка, размеры и содержимое окон принадлежат потребителю. Старые настройки продуктов сохраняются для миграции, но не переопределяют выбранное общее оформление.

В том же окне настраиваются прилипание к экрану и окнам dmicher, выравнивание углов и центров. Порог переноса — 12 CSS-пикселей; программное восстановление положения и изменение размера не включают прилипание. Собственный handle переноса отмечается `data-dmicher-window-drag`; встроенная панель — `data-dmicher-docked="true"`. Обычные заголовки Foundry поддерживаются автоматически.

Premium-доступ к Generics позволяет импортировать CSS-файл до 64 КиБ поверх встроенного стиля. Его применяет реальная премиальная реализация `resolveCustomStyles` через общий мост. Без доступа слой снимается, а сохранённый файл остаётся. CSS ограничен окнами dmicher. Подробные контракты, миграция и поддерживаемые правила — в [документации оформления](docs/appearance-api.md).
...
### Общий интерфейс справки

`generics.help.createHelpApplication` создаёт окно с раскрывающимся деревом, тремя закреплёнными нижними страницами и изменяемой перетаскиванием шириной меню. Потребитель передаёт собственные страницы RU/EN, идентификаторы, заголовки, ссылки и содержимое, в том числе страницы автора, благодарностей и Premium. Generics не хранит и не подменяет справку других модулей.

`generics.help.bindSettingHelp` добавляет к указанным настройкам значки вопроса: подсказку при наведении и переход на страницу/якорь при нажатии. Справка содержит краткие действия пользователя и объяснение настроек; архитектура и инструкции разработчику остаются в документации репозитория. Точный контракт — в [документации справки](docs/help-api.md).
